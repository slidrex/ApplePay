...
public class EffectDatabase
{
    ...
    public Dictionary<string, EffectTemplate> Effects 
    {
        get
        {
            ...
            {
                ["slowness"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(GetSprite("Slowness.png"), "Slowness", "Movement speed decreased by " + (Level * 0.1f) * 100 + "%.", ColorizeRoman(Level, EffectColor.Negative), new Pay.UI.UIManager.TextField(textConfigDatabase.Find("lore"), "Agent pidorasik makes you slow)))")), 10, States.VelocityChanger(-0.1f * Level)),
                ["speed"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(GetSprite("Slowness.png"), "Speed", "Movement speed increases by " + (Level * 0.1f) * 100 + "%.", ColorizeRoman(Level, EffectColor.Positive)), byte.MaxValue, States.VelocityChanger(0.1f * Level)),
                ["move_constraint"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(GetSprite("Interact.png"), "Move Constraint", "Constraints your movement abilities.", ColorizeRoman(Level, EffectColor.Negative)), 1, States.MoveConstraint()),
                ["speed_hack"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(GetSprite("Interact.png"), "Hack Haste", "Increases hack speed by "+ (Level * 0.1f) * 100 + "%.", ColorizeRoman(Level, EffectColor.Negative)), byte.MaxValue, States.HoldingHackSpeedChanger(0.1f * Level)),
                ...
                ["regeneration"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(null, "Regeneration", "Restores " + 5 + " health every " + 5/Level +" seconds.",  ColorizeRoman(Level, EffectColor.Positive)), byte.MaxValue, States.ChangeHealth(5, 5/Level)),
                ["regeneration%"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(null, "Regeneration", "Restores " + 5 + "% health every " + 5/Level +" seconds.",  ColorizeRoman(Level, EffectColor.Positive)), byte.MaxValue, States.ChangeHealth(5f, 5/Level)),
                ["decay"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(null, "Decay", "Loses " + 2 + " health every " + 5/Level +" seconds.",  ColorizeRoman(Level, EffectColor.Negative)), byte.MaxValue, States.ChangeHealth(-2, 5/Level)),
                ["decay%"] = new EffectTemplate(new PayWorld.EffectController.EffectDisplay(null, "Decay", "Loses " + 5 + "% health every " + 5/Level +" seconds.",  ColorizeRoman(Level, EffectColor.Negative)), byte.MaxValue, States.ChangeHealth(-0.05f, 5/Level))
            };
            return effects;
        }
    }
